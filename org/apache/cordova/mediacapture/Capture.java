...
public class Capture extends CordovaPlugin {
    ...
    class C00881 implements Runnable {
        ...
        C00881(Intent intent, Capture capture) {
        ...
        public void run() {
            try {
                ...
                try {
                    ...
                    LOG.m0d(Capture.LOG_TAG, "Can't write to external media storage.");
                    try {
                        uri = this.val$that.cordova.getActivity().getContentResolver().insert(Media.INTERNAL_CONTENT_URI, values);
                    } catch (UnsupportedOperationException e2) {
                        LOG.m0d(Capture.LOG_TAG, "Can't write to internal media storage.");
    ...
    class C00903 implements Runnable {
        ...
        C00903(Intent intent, Capture capture) {
            this.val$intent = intent;
            this.val$that = capture;
        }
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent intent) {
        if (resultCode == -1) {
            if (requestCode == 0) {
                this.cordova.getThreadPool().execute(new C00881(intent, this));
            } else if (requestCode == CAPTURE_IMAGE) {
                this.cordova.getThreadPool().execute(new C00892(this));
            } else if (requestCode == CAPTURE_VIDEO) {
                this.cordova.getThreadPool().execute(new C00903(intent, this));
            }
